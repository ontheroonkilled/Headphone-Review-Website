...
if (isset($_POST['guncelle'])) {
    ...
    if (!empty($resim_yolu)) {
        $sql = $baglan->prepare("UPDATE kulakliklar SET baslik = ?, icerik = ?, resim = ?, zaman = ? WHERE id = ?");
        $sql->bind_param("ssssi", $baslik, $icerik, $resim_yolu, $zaman, $duzen_id);
    } else {
        $sql = $baglan->prepare("UPDATE kulakliklar SET baslik = ?, icerik = ?, zaman = ? WHERE id = ?");
...
$sql = "SELECT * FROM kulakliklar WHERE `id`=$id;";
